...
const PatientDashboard = () => {
  ...
  return (
    <div style={{ padding: "2rem", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ maxWidth: 700, width: "100%", background: "#e3f2fd", borderRadius: 12, boxShadow: "0 1px 6px rgba(25,118,210,0.08)", padding: "1.5rem", marginBottom: "2rem" }}>
        <h3 style={{ marginBottom: "1rem" }}>Specialist Feedback</h3>
        <ul style={{ paddingLeft: "1.5rem" }}>
          {feedback.length === 0 ? (
            ...
            feedback.map((f, idx) => (
              <li key={idx} style={{ marginBottom: "1rem" }}>
                <b>Comment:</b> {f.comments}<br />
                <span style={{ color: '#555', fontSize: '0.95em' }}>
                  {f.specialist_id ? `From Specialist #${f.specialist_id}` : ''} &nbsp;|&nbsp; {f.reading_id ? `Related to Reading #${f.reading_id}` : ''}
                </span>
              </li>
            ))
          )}
