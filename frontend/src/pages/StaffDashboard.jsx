...
const StaffDashboard = () => {
  ...
  const handleThresholdSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      // Decode user ID from token (assuming format token-<user_id>)
      let staffId = 1;
      if (token && token.startsWith("token-")) {
        staffId = parseInt(token.split("-")[1]);
      }
      await api.post("/thresholds/", {
    ...
    <div style={{ padding: "2rem", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "2rem" }}>
        ...
        <div style={{ flex: 1, minWidth: 280 }}>
          ...
          <h4 style={{ marginTop: "2rem" }}>Current Thresholds</h4>
          {thresholds.map((t) => (
            <div key={t.threshold_id} style={{ background: "#f8f8f8", borderRadius: 8, padding: "0.75rem", marginBottom: 8 }}>
